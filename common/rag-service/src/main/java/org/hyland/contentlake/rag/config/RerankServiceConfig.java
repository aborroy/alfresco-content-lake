...
/**
 * Registers exactly one {@link RerankService}, in order of precedence:
 * <ul>
 *   <li>{@link TeiCrossEncoderRerankService} when {@code rag.reranker.url} is set to a non-blank value</li>
 *   <li>{@link LlmRerankService} when no url is set and {@code rag.reranker.enabled} is true</li>
 *   <li>{@link NoOpRerankService} otherwise (keeps retrieval ordering unchanged)</li>
 * </ul>
 *
 * <p>A TEI url always takes precedence; {@code rag.reranker.enabled} only gates LLM-vs-NoOp when
 * no url is set. Beans are declared in this class so that {@code @ConditionalOnMissingBean} is
 * evaluated after the property-conditional beans, avoiding class-scan ordering issues.
...
public class RerankServiceConfig {
    ...
    @Bean
    @ConditionalOnExpression("'${rag.reranker.url:}' == '' and ${rag.reranker.enabled:false}")
    public RerankService llmRerankService(ChatModel chatModel, RagProperties ragProperties) {
        return new LlmRerankService(chatModel, ragProperties);
    }
